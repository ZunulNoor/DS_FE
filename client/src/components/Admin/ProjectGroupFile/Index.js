...
export const Index = () => {
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios.delete(`${AppRoute}api/delete-project-group/${id}`);
                loadData();
        ...
        <div className="container-fluid">
            <div className="box">
                <div className="box-header">
                    <h3 className="box-title">Project Group File</h3>
                    <div className="container-fluid">
                        <div className="row">
                            <div className="form-group text-right">
                                <Link to="/projectgroupfile/new" className="btn btn-primary">
                                    ...
                                    <table id="example2" className="table table-bordered table-striped table-condensed dataTable" role="grid" aria-describedby="example1_info">
                                        ...
                                        <tbody>
                                            {searchJsonResult.map((val, key) =>
                                                <tr key={key}>
                                                    <td>{val.id}</td>
                                                    <td>{val.project_group}</td>
                                                    <td className="text-center">
                                                        <Link to={`/projectgroupfile/edit/${val.id}`} className="btn btn-xs btn-primary">
